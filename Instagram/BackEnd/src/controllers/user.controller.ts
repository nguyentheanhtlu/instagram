  ...
  async updateProfile(req, res, next) {
    try {
      ...
      const userExists = await UserRepo.createQueryBuilder("user")
        .where("(user.email = :email OR user.username = :username) AND user.id != :userId", {
          email,
          username,
      ...
      if (userExists) {
        return next(createError(400, "Email or username already in use"));
      }
      // Xử lý cập nhật ảnh đại diện
      if (req.file) {
        if (user.profile_picture) {
          // Xóa ảnh cũ nếu có
          const oldAvatarPath = path.join(__dirname, '../../../uploads/', path.basename(user.profile_picture));
